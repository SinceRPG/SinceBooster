...
def main():
    ...
    if is_start:
        send_initial_message(config, author, commit_msg, f"SinceBooster v{version} • {commit_hash}")
    else:
        ...
        if is_fail:
            patch_message(config, msg_id, "Build Failed ❌", config["FAIL_DESC"], config["COLOR_FAIL"],
                          f"v{version} • Failed • {commit_hash}")
        else:
            ...
            patch_message(config, msg_id, "Build Successful! 🚀", commit_msg, config["COLOR_SUCCESS"],
                          f"v{version} • {commit_hash}", path)
